...
class TTSRepositoryAdapter implements TTSRepository {
  ...
  final _logger = Logger('TTSRepositoryAdapter');
  ...
  Future<List<Language>> getTTSLanguages() async {
    try {
      _logger.info('TTSRepositoryAdapter: Getting available languages...');
      // Convert language strings to Language objects
      final languageCodes =
          await _textToSpeechRepository.getAvailableLanguages();
      _logger.info(
        'TTSRepositoryAdapter: Received ${languageCodes.length} language codes',
      );

      if (languageCodes.isEmpty) {
        _logger.info(
          'TTSRepositoryAdapter: No languages received, adding fallback languages',
        );
      ...
      _logger.info(
        'TTSRepositoryAdapter: Converted ${result.length} languages',
      );
      for (final lang in result) {
        _logger.info(
          'TTSRepositoryAdapter: Language ${lang.name} (${lang.code})',
        );
      }
      return result;
    } catch (e) {
      _logger.info('TTSRepositoryAdapter: Error getting languages: $e');
  ...
  String _formatTTSLanguageCode(String languageCode) {
    _logger.info(
      'TTSRepositoryAdapter: Formatting TTS language code: $languageCode',
    );
  ...
  Future<void> speak(String text, String languageCode) async {
    ...
    _logger.info(
      'TTSRepositoryAdapter: Speaking with language code: $languageCode -> $formattedCode',
    );
